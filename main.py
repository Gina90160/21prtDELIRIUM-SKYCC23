...
def run():
  ...
  for filelistname in file_list:
      if filelistname.isdigit():
        if num < int(filelistname):
          num = int(filelistname)
  
  file_list = os.listdir('./'+str(num)+'/')
  for filelistname in file_list:
    if filelistname[-2:] == 'py' and filelistname != 'main.py':
  ...
  for node in parsed.body:
    if isinstance(node, ast.ImportFrom):
      for alias in node.names:
        name = alias.name
        import_lines.append(
          astor.to_source(node).replace(node.module, name).strip())
