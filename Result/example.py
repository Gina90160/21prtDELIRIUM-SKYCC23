from ScalarFormatter import ScalarFormatter
from rc import rc
import numpy as np
...
def autocorrelate_fb(signals_mat, snap_size):
  ...
  out_matrix = 1.0 / snap_size * np.transpose(signals_mat) @ np.conjugate(
    signals_mat)
  return 0.5 * out_matrix + (0.5 /
                             snap_size) * d_J @ np.conjugate(out_matrix) @ d_J


# antenna array creation, equally placed around zero
def gen_antenna_vec(lambda_separation, n_rx):
  return np.linspace(-(n_rx - 1) * lambda_separation / 2,
                     (n_rx - 1) * lambda_separation / 2, n_rx)
...
def gen_real_case_signal():
  ...
  for idx in range(n_rx):
    cx_signal_vec = np.array(
      (ampl_diff_vec[idx] * np.cos(angle_vec + phase_diff_vec[idx]) +
       (1j * ampl_diff_vec[idx] * np.sin(angle_vec + phase_diff_vec[idx]))))

    if add_noise_flag:
      awgn = np.random.normal(
        0, 0.05, snap_size) + (1j * np.random.normal(0, 0.05, snap_size))
...
def gen_signals_mat(n_rx,
                    antenna_vec,
                    n_sources,
                    n_samples,
                    source_theta_vec=None,
                    source_power_vec=None,
                    power_diff_vec=None,
                    snr_db=float('inf'),
                    print_params=False):
  ...
  if (source_power_vec is None):
    ...
    source_power_vec = np.sqrt(
      1 / 2) * (np.random.randn(n_sources) + np.random.randn(n_sources) * 1j)
  ...
  for sample_idx in range(n_samples):
    ...
    for source_idx in range(n_sources):
      ...
      array_sync_sample_vec = array_sync_sample_vec + phase * source_power_vec[
        source_idx] * array_response_vector(antenna_vec,
                                            source_theta_vec[source_idx])
    signals_mat[sample_idx, :] = array_sync_sample_vec
    # add white gaussian noise
  for sample_vec_idx in range(signals_mat.shape[1]):
    signals_mat[:,
                sample_vec_idx] = add_awgn_vec(signals_mat[:, sample_vec_idx],
                                               snr_db)
...
def add_awgn_vec(in_vec, snr_db):
  ...
  noise_vec = (np.sqrt(imp * noise_pow / 2)) * (
    np.random.randn(len(in_vec)) + 1j * np.random.randn(len(in_vec)))
...
arr_response_mat, arr_response_mat_trans = gen_array_response_mat(
  pspec_length, n_rx, theta_vec, antenna_vec)
...
# generate signal
sig_mat, source_theta_vec = gen_signals_mat(n_rx, antenna_vec, n_sources,
                                            n_samples, source_theta_vec,
                                            source_power_vec, power_diff_vec,
                                            snr)
...
pspec_vec = calc_aoa_music(n_sources, n_rx, autocorr_mat, arr_response_mat,
                           arr_response_mat_trans)
...
for rx_power_coeff in rx_power_coeff_vec:
  ...
  plt.plot(np.degrees(theta_vec),
           pspec_vec,
           linewidth=1,
           label='%1.2f' % rx_power_coeff)
...
plt.grid(which="both", ls="-")
plt.tight_layout()
plt.savefig("figs/aoa_sim_ant_spacing_mc_rmse.png",
            dpi=600,
            bbox_inches='tight')
plt.show()
...
plt.grid(which="both", ls="-")
plt.tight_layout()
plt.savefig("figs/aoa_sim_rx_pow_diff_mc_rmse.png",
            dpi=600,
            bbox_inches='tight')
plt.show()
